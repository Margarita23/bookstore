    ...
    describe 'POST #create' do
      ...
      describe 'line_items new quantity is valid' do
        ...
        it 'flash alert if @line_item is nil' do
          allow(CheckingQuantityService).to receive_message_chain('new.call').and_return(nil)
          subject
          expect(flash[:alert]).to eq I18n.t(:books_not_added)
        end
        it 'flash alert after save' do
          allow(CheckingQuantityService).to receive_message_chain('new.call').and_return p
